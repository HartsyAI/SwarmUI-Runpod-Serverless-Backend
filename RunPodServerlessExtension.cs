...
public static class RunPodPermissions
{
    ...
    public static readonly PermInfo PermUseRunPod = Permissions.Register(new PermInfo("use_runpod_serverless", "Use RunPod Serverless", "Allows using RunPod's serverless GPU endpoints for image generation.", PermissionDefault.POWERUSERS, RunPodPermGroup));
}
...
public class RunPodServerlessExtension : Extension
{
    ...
    public override void OnInit()
    {
        ...
        Program.Backends.RegisterBackendType<RunPodServerlessBackend>("runpod_serverless", "RunPod Serverless", "Serverless GPU inference via RunPod with direct SwarmUI API access. Supports on-demand scaling and cost-effective generation.", CanLoadFast: true);
        ...
        try
        {
            ...
            {
                UserUpstreamApiKeys.Register(new UserUpstreamApiKeys.ApiKeyInfo(KeyType: "runpod_api", JSPrefix: "runpod", Title: "RunPod", CreateLink: "https://www.runpod.io/console/user/settings",
        ...
        try
        {
            if (!ModelsAPI.ExtraModelProviders.ContainsKey("runpod_serverless"))
            {
                ModelsAPI.ExtraModelProviders["runpod_serverless"] = (string subtype) =>
                {
                    ...
                    Dictionary<string, Newtonsoft.Json.Linq.JObject> result = sets.Aggregate((a, b) => a.Union(b).PairsToDictionary(false));
        ...
        try
        {
            T2IEngine.PreGenerateEvent += (p) =>
            {
                ...
                foreach (RunPodServerlessBackend b in Program.Backends.RunningBackendsOfType<RunPodServerlessBackend>())
                {
                    ConcurrentDictionary<string, Dictionary<string, Newtonsoft.Json.Linq.JObject>> rem = b.RemoteModels;
                    if (rem is null)
                    {
                        Logs.Verbose($"[RunPodServerless] PreGenerateEvent: backend #{b.BackendData?.ID} has no RemoteModels, skipping");
                        continue;
                    }
                    bool found = rem.Values.Any(dict => dict.ContainsKey(requestedModel) || dict.ContainsKey(requestedModel.EndsWith(".safetensors", StringComparison.OrdinalIgnoreCase) ? requestedModel[..^".safetensors".Length] : requestedModel)
                        || dict.Keys.Any(k => k.Equals(requestedModel.AfterLast('/'), StringComparison.OrdinalIgnoreCase)) || dict.Keys.Any(k => k.Equals((requestedModel.EndsWith(".safetensors", StringComparison.OrdinalIgnoreCase) ? requestedModel[..^".safetensors".Length] : requestedModel).AfterLast('/'), StringComparison.OrdinalIgnoreCase)));
                    if (found)
                    {
